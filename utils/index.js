...
export const shortenAddress = (address = "", start = 5, chars = 5) =>
  address.length > 14
    ? `${address.substring(0, start)} ... ${address.substring(
...
export const humanReadableNumber = (num, minimumFractionDigits = 2) =>
  parseFloat(num)
    .toLocaleString("en-GB", { minimumFractionDigits })
    .replace(/\.0+$/, "");

export const BigNumberConverter = (value) => {
  const valueConverter = parseFloat(value);
  const result = new BigNumber(valueConverter)
    .decimalPlaces(12, BigNumber.ROUND_DOWN)
    .toString();
  return parseFloat(result);
};

export const getCoingeckoIdFromContractAddress = (contractAddress) =>
  KNOWN_TOKENS[contractAddress] &&
  KNOWN_TOKENS[contractAddress]?.coingeckoId &&
  KNOWN_TOKENS[contractAddress]?.coingeckoId;
